...
function install(){
 ...
 document.addEventListener("click",event=>{const close=event.target.closest?.(`#${C.ids.close}`);if(!close)return;const n=selected(),data=loadBets(),row=n?data[String(n)]:null;if(!n||!row||row.status!=="ativo"||locked(n))return;const ids=currentBetIds();row.gameIds=ids;row.totalInvested=ids.length*(Number(row.unitPrice)||0);row.savedAt=now();row.updatedAt=now();data[String(n)]=row;saveBets(data)},true);
